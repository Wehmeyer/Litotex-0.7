...
{
    $length= (strlen(char) < 5 ? 6 : strlen(char));
    $password="";
...
function get_duration_time ($startX,$startY,$endX,$endY,$Sol_speed)
{
    ...
    $an_time= TIMESTAMP + round($duration*$op_land_duration * $Sol_speed / 100);
...
function get_race_id_from_group($group_id) 
{
    ...
    return get_race_id_from_countrie($countrie_id);
}
...
function get_countrie_name_from_group_id($group_id,$inclu_koords=0) 
{
    ...
    return get_countrie_name_from_id($is_id,$inclu_koords);
}
...
{
    return "<a href=\"" . LITO_ROOT_PATH_URL . "members/members.php?action=profile&id=$user_id\">$username</a>";
}

function generate_messagelink($username,$show_icon=0)
{
    $module=get_modulname(6);
    $msg_modul_org= LITO_ROOT_PATH_URL . $module[0]."/".$module[1];
    $icon_url="";
    if ($show_icon==1)
    {
